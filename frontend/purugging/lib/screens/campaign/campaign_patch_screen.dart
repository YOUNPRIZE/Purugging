...
class CampaignPatchScreen extends StatefulWidget {
  const CampaignPatchScreen({super.key, required this.campaignInfo});
  final CampaignModel campaignInfo;

  @override
  State<CampaignPatchScreen> createState() => _CampaignPatchScreenState();
}

class _CampaignPatchScreenState extends State<CampaignPatchScreen> {
  // 변수 - 폼과 연결된 변수들
  ...
  Future<void> createCampaign(context) async{
    if (titleController.text.isEmpty) {
    ...
    } else if (numberController.text.isEmpty || int.parse(numberController.text) > 50) {
      ...
      bool result = await CampaignServices.patchCampaign(
        {        
          "title" : titleController.text, //String
          "content" : contentController.text, //String
          "date" : DateFormat("yyyy-MM-ddTHH:mm:ss").format(date), //yyyy-MM-dd'T'HH:mm:ss
          "max_participant_number" : int.parse(numberController.text) //Long
        }, widget.campaignInfo.campaign_id
      );
      if (result) {
  ...
  void initState() {
    super.initState();
    titleController.text = widget.campaignInfo.title;
    contentController.text = widget.campaignInfo.content;
    numberController.text = widget.campaignInfo.max_participant.toString();
    date = widget.campaignInfo.date;
  }
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [
                // 제목 입력
                InputTitleWidget(controller: titleController, focusNode: titleFocusNode,),
                const SizedBox(
                  height: 30,
                ),
                // 내용 입력
                InputContentWidget(controller: contentController, focusNode: contentFocusNode,),
                const SizedBox(
                  height: 30,
                ),
                // 일시 입력
                ...
                // 최대 인원수 입력
                InputNumberWidget(controller: numberController, focusNode: numberFocusNode,),
                const SizedBox(
                  height: 40,
                ),
                Flex(
